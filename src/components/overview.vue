<template>
    <div class="container" style="margin-top: 3rem;">
      <h2>Overview of Yeti Bitcoin Storage</h2>
      <p>Yeti is currently pre-alpha. This means that not only is the code not well tested, but it isn't even funtional yet. We would love help testing and code review and hope to have something usable and reasonably trustworthy within a few weeks.</p>
      ...
      <ul>
          ...
          <li>Minimal software beyond bitcoin core. Bitcoin core is far and away the most trustworthy bitcoin software. Unfortunately it doesn’t yet provide a user friendly interface for establishing a multisig address using an offline device or display and accept private keys in a human transcribable format. As bitcoin core adds these features we will reduce the Yeti code that provides these features now. Eventually we hope Yeti will be entirely unnecessary and only trusting bitcoin core will be required.</li>
          <li>Open source and easily audited. One of the reasons bitcoin core is trustworthy is that it is the most scrutinized software. This makes it the least likely to contain a critical security flaw that hasn’t been identified and fixed. Yeti will never be as trustworthy, but by minimizing the amount of code and primarily using python scripts and console commands and a tiny amount of JavaScript the effort required to verify that Yeti is performing as expected is minimized.</li>
          ...
          <li>Counterfiet prevention. The only way to be certian that your balance represents genuine bitcoin is to use a bitcoin full node - in fact that is the primary purpose of a bitcoin full node - to verify that the bitcoin balance is correct and full of only genuine bitcoins. Any solution that does not involvle a full node requires you trust someone else to tell you if you have real bitcoin.</li>
          <li>The process can be comelpted by non technologists with minimal effort.</li>
          ...
          <li>Instructions for recovering the bitcoin are included with every copy of the private keys to reduce the liklihood of loss.</li>
      </ul>
      ...
      <ul>
          ...
          <li>Privacy. While using bitcoin core over Tor does provide signficant privacy advantages over many cold storage solutions using multisig is not very common and a 3 of 7 multisig is even less common. This means that someone could look at the blockchain and infer that the owner of the coins is probobly using yeti for cold storage. This will eventually be fixed through changes to bitcoin and it is worth the security and recovery benefit to use multisig.</li>
      </ul>
